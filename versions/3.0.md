...
## <a href="discovery"></a>OpenAPI and Documentation Discovery

Service providers may want to make an OpenAPI description discoverable from (some) resources provided by the API. One typical example could be an API's "home" resource (the intended starting point of the API), which for example might use [a standardized home document format](https://tools.ietf.org/html/draft-nottingham-json-home-04). (This home document in turn may be made discoverable [by "home" links](https://tools.ietf.org/html/draft-wilde-home-xml-04#section-4.3.1) from other resources provided by the API.)

In such a scenario, it can help if there is a predictable way in which API description and documentation is made discoverbale from the API itself. For improving the discoverability of description and documentation available for an API, the following recommendations are made:

* It is recommended that links from API resources to the OpenAPI description of the API are using [the "service-desc" link relation type](https://tools.ietf.org/html/draft-wilde-service-link-rel-01#section-4.2).
* It is recommended that additional (primarily human-readable) documentation, which often may be the documentation that is linked from the OpenAPI object via the "externalDocs" field, is linked from API resources by using [the "service-doc" link relation type](https://tools.ietf.org/html/draft-wilde-service-link-rel-01#section-4.1).
