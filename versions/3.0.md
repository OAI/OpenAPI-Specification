...
In all cases, _variables_ from requests and responses MAY be substituted for link generation.
...
Request parameter      | `$request.path.id`        | `/users/{$request.path.id}`            | Request parameters MUST be declared in the `parameters` section of the operation to be used in substitution.  This includes substitution in request headers.
Request body           | `$request.body`    | `/users/{$request.body#/user/uuid}` | In operations which accept payloads, references MAY be made to portions of the `requestBody` or the entire body.
Request URL            | `$url`            | `/track?url={$url}`               | 
Response value         | `$response.body`       | `{$response.body#/uuid}`                | The `$response` syntax can access only the payload in the response.  
...
For responses, the response payload MAY be accessed with the `$response` syntax.
