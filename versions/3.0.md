...
* Links from API resources to the OpenAPI description of the API should use [the "service-desc" link relation type](https://tools.ietf.org/html/draft-wilde-service-link-rel-01#section-4.2).
* Additional (primarily human-readable) documentation, which often may be the documentation that is linked from the OpenAPI object via the "externalDocs" field, should be linked from API resources by using [the "service-doc" link relation type](https://tools.ietf.org/html/draft-wilde-service-link-rel-01#section-4.1).
