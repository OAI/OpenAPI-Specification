...
```yaml
# an arbitray binary file:
content:
    application/octet-stream: {}
```

Binary content transferred wiht base64 encoding:
...
* If the property is a `type: string` with a `contentEncoding` of `base64` or `base64url`, the default Content-Type is `application/octet-stream`
