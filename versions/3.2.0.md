...
<a name="pathItemUndefinedQueryParameters"></a>undefinedQueryParameters | `boolean` | Sets the default behavior of the same field at the [Operation Object](#operationUndefinedQueryParameters) level; see that field for the description of its effects.  Default value is `true`.
...
<a name="operationUndefinedQueryParameters"></a>undefinedQueryParameters | `boolean` | If `false`, the presence of any query string parameter not defined by this operation or its [Path Item](#pathItemParameters) MUST be treated as an error.  If not specified, then the value of this field at the [Path Item](#pathItemUndefinedQueryParameters) level is used.
