...
<a name="pathItemAllowUndefinedQueryParameters"></a>allowUndefinedQueryParameters | `boolean` | Sets the default behavior of the same field at the [Operation Object](#operationAllowUndefinedQueryParameters) level; see that field for the description of its effects.  Default value is `true`.
...
<a name="operationAllowUndefinedQueryParameters"></a>allowUndefinedQueryParameters | `boolean` | If `false`, the presence of any query string parameter not defined by this operation or its [Path Item](#pathItemParameters) MUST be treated as an error.  If not specified, then the value of this field at the [Path Item](#pathItemAllowUndefinedQueryParameters) level is used.
